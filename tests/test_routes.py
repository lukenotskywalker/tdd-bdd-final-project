...
class TestProductRoutes(TestCase):
    ...
    def test_method_not_allowed(self):
        ...
        # Attempt to use an unsupported method (e.g., POST) on a GET-only endpoint
        response = self.client.delete(BASE_URL)
        
        # Check that the response status code is 405
    ...
    def test_find_by_price(self):
        ...
        # Use the find_by_price method to retrieve products with the test price
        found_products = Product.find_by_price(test_price).all()
        
        # Check that the correct number of products is returned
        expected_count = len([p for p in products if p.price == test_price])
        self.assertEqual(len(found_products), expected_count)
        
        # Verify that each returned product has the correct price
